...
**На фото основные таблицы БД**
![ER таблица базы данных](ER.jpg)

### tickets_ticket

Основная модель для хранения данных заявки, её статуса, приоритета и связей с создателем, исполнителем и группой.

### tickets_statuslog

Хранит историю изменений статусов заявок с комментариями и временными метками для аудита.

### users_customuser

Настраиваемая модель пользователя, содержащая данные для аутентификации и базовую личную информацию.

### users_profile

Связывает пользователя с конкретной организацией, а также определяет его роль и контактные данные.

### company_organization

Представляет юридическое лицо или подразделение, имеющее своего главного менеджера.

### company_workergroup

Определяет конкретную рабочую группу или команду внутри организации под руководством менеджера.

### company_workergroup_workers

Техническая таблица для связи «многие ко многим», прикрепляющая пользователей к рабочим группам.
