...
class TicketListView(django.views.generic.ListView):
    ...
    def get_queryset(self):
        ...
        return qs.sort(status="asc", priority="asc", created="asc")

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["group"] = users.models.WorkerGroup.objects.get(pk=self.kwargs["pk"])
        return context
...
class TicketDetailView(django.views.generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["logs"] = self.object.status_logs.all().order_by(f"-{tickets.models.StatusLog.timestamp.field.name}")
        return context
    ...
    def get_context_data(self, **kwargs):
        ...
        for group in users.models.WorkerGroup.objects.filter(
            ...
            groups.append(
                {
                    "group": group,
                    "tickets": tickets_qs.filter(group=group),
                }
            )
